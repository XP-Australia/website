  ...
  return (
    <Div article={props.article}>
      <LogoWrapper article={props.article}>
        {props.article && (
          <NameLink href="/">
            <NameArticle article={props.article}>XP Australia</NameArticle>
          </NameLink>
        )}
        {!props.article && <Name article={props.article}>XP Australia</Name>}
        {!props.article && <Role>Startup Solutions</Role>}
      </LogoWrapper>
      <div>
        <SocialLinks article={props.article}>
          <SocialLink
            href="mailto:contact@auxp.com.au"
            onMouseOver={() => showTooltip('Mail')}
            onFocus={() => showTooltip('Mail')}
            onMouseLeave={hideTooltip}
            onBlur={hideTooltip}
            aria-label="Send an email to XP Australia"
          >
            <SvgWrapper>
              <InlineSvg>
